...
def _student_full_record(student_id: str) -> dict | None:
    ...
    materials_read  = fetchone("SELECT COUNT(*) AS c FROM student_progress WHERE student_id = %s", [student_id])
...
async def admin_dashboard(request: Request):
    ...
    readiness_row   = fetchone("SELECT COALESCE(AVG(score), 0) AS avg FROM assessment_submissions")
...
async def faculty_dashboard(request: Request):
    ...
    my_content   = fetchone("SELECT COUNT(*) AS c FROM content_modules WHERE author_id = %s", [auth.user_id])
    my_assess    = fetchone("SELECT COUNT(*) AS c FROM assessments WHERE author_id = %s",      [auth.user_id])
    pending_cont = fetchone("SELECT COUNT(*) AS c FROM content_modules WHERE author_id = %s AND status = 'PENDING'", [auth.user_id])
...
async def admin_analytics_list(request: Request):
    auth = login_required(request)
    if auth.role != "ADMIN": return forbidden()
...
async def admin_analytics_detail(request: Request, student_id: str):
    auth = login_required(request)
    if auth.role != "ADMIN": return forbidden()
...
async def faculty_analytics_list(request: Request):
    auth = login_required(request)
    if auth.role != "FACULTY": return forbidden()
    return ok(_analytics_list(request))
...
async def faculty_analytics_detail(request: Request, student_id: str):
    auth = login_required(request)
    if auth.role != "FACULTY": return forbidden()
    record = _student_full_record(student_id)
    return ok(record) if record else not_found("Student not found")
...
@mobile_prog_router.get("/dashboard")
async def student_dashboard(request: Request):
    ...
    readiness = _calc_readiness(auth.user_id)

    subjects = fetchall("SELECT s.id, s.name, s.color FROM subjects s WHERE s.status = 'APPROVED' ORDER BY s.name")
    subject_breakdown = []
    for s in subjects:
        sid           = str(s["id"])
        total_content = fetchone("SELECT COUNT(*) AS c FROM content_modules WHERE subject_id = %s AND status = 'APPROVED'", [sid])
        completed     = fetchone("SELECT COUNT(*) AS c FROM student_progress sp JOIN content_modules cm ON cm.id = sp.content_id WHERE sp.student_id = %s AND cm.subject_id = %s", [auth.user_id, sid])
        assess_avg    = fetchone(
...
async def student_progress(request: Request):
    auth = login_required(request)
    if auth.role != "STUDENT": return forbidden()
    record = _student_full_record(auth.user_id)
