...
-- schema.sql
-- PostgreSQL 14+
-- Clean schema: subjects > modules (hierarchical) > materials (content)
-- ============================================================
...
DROP VIEW IF EXISTS v_materials_with_meta       CASCADE;
...
-- ────────────────────────────────────────────────────────────
-- 5. MODULES  (hierarchical units within a subject: module → submodule)
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- 10. STUDENT PROGRESS  (material read tracking)
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS student_progress (
    id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    student_id   UUID        NOT NULL REFERENCES users(id)      ON DELETE CASCADE,
    content_id   UUID        NOT NULL REFERENCES materials(id)  ON DELETE CASCADE,
...
-- ────────────────────────────────────────────────────────────
-- 15. QUESTIONS BANK  (standalone bank)
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- 16. ENROLLMENTS  (student ↔ subject)
-- ────────────────────────────────────────────────────────────
...
DROP TRIGGER IF EXISTS trg_materials_last_updated ON materials;
CREATE TRIGGER trg_materials_last_updated
    BEFORE UPDATE ON materials FOR EACH ROW EXECUTE FUNCTION set_last_updated();
...
CREATE TRIGGER trg_request_changes_last_updated
    BEFORE UPDATE ON request_changes FOR EACH ROW EXECUTE FUNCTION set_last_updated();
...
RETURNS TABLE (
    ...
    SELECT
        s.name,
        COALESCE(MAX(CASE WHEN a.type = 'PRE_ASSESSMENT'  THEN sub.score END), 0),
        COALESCE(MAX(CASE WHEN a.type = 'POST_ASSESSMENT' THEN sub.score END), 0),
...
BEGIN
    ...
    LOOP
        IF rec.day = expected THEN
            streak   := streak + 1;
            expected := expected - 1;
        ELSIF rec.day < expected THEN EXIT;
        END IF;
    END LOOP;
    RETURN streak;
END;
...
RETURNS INT LANGUAGE sql STABLE AS $$
    SELECT (
        (SELECT COUNT(*) FROM materials  WHERE status IN ('PENDING','REMOVAL_PENDING')) +
...
SELECT
    ...
    (SELECT COUNT(*) FROM materials WHERE status IN ('PENDING','REMOVAL_PENDING'))              AS pending_materials,
...
-- Materials with joined names
CREATE OR REPLACE VIEW v_materials_with_meta AS
SELECT
    mat.*,
    s.name                                        AS subject_name,
    m.title                                       AS module_title,
...
FROM materials mat
LEFT JOIN subjects s ON s.id = mat.subject_id
LEFT JOIN modules  m ON m.id = mat.module_id
LEFT JOIN users    u ON u.id = mat.author_id;
...
-- Request changes with revision info
...
-- Whitelist with user account status
...
-- Subjects with module & material counts
...
SELECT
    ...
    (SELECT COUNT(*) FROM materials WHERE subject_id = s.id AND status = 'APPROVED')       AS approved_materials_count,
    (SELECT COUNT(*) FROM assessments WHERE subject_id = s.id AND status = 'APPROVED')     AS approved_assessments_count
FROM subjects s
